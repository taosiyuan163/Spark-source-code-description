 ...
 */
// ShuffledRDD，MapPartitionsRDD，CoGroupedRDD，HadoopRDD等都是继承RDD
// 并复写了各自需要的部分功能代码
abstract class RDD[T: ClassTag](
  ...
  final def partitions: Array[Partition] = {
    // 先遍历所有checkpoint的每个Rdd的每个分区并提取出来
    checkpointRDD.map(_.partitions).getOrElse {
      // 如果这个RDD分区之前并没有被checkpoint则会检查Array[Partition]是否为空
      if (partitions_ == null) {
        // 返回一个Array[Partition]用来存放这个RDD的partitions
        // 不同的RDD算子都会用不同的逻辑覆写getPartitions
        partitions_ = getPartitions
        // 生成一个0开始的计数器与partition的拉链操作，是一个有序的二元tuple（类似于(p1,0),(p2,1)...）
        // 这里主要用来做类似断言的判断分区索引是否相等
        partitions_.zipWithIndex.foreach { case (partition, index) =>
          // 类似于之前章节提到过的assert，用作参数限制判断
          require(partition.index == index,
            s"partitions($index).partition == ${partition.index}, but it should equal $index")
        }
      }
      // 如果没有被checkpointRDD就会返回这个partitions_
      partitions_
    }
  }
  ...
  private[spark] def computeOrReadCheckpoint(split: Partition, context: TaskContext): Iterator[T] =
  {
    if (isCheckpointedAndMaterialized) {
      ...
      // 每种RDD都不一样，以shuffledRDD为例 ,会new一个reader来拉取各个分区的数据
    ...
    RDDOperationScope.withScope(sc, "checkpoint", allowNesting = false, ignoreParent = true) {
      if (!doCheckpointCalled) {
        ...
        if (checkpointData.isDefined) {
          ...
          // 把需要checkpoint的RDD最终做doCheckpoint()处理
          checkpointData.get.checkpoint()
        } else {
          // 如果当前RDD没有设置checkpoint就继续递归调用每个父RDD有没有设置Checkpoint
          dependencies.foreach(_.rdd.doCheckpoint())
        }
      }
    }
