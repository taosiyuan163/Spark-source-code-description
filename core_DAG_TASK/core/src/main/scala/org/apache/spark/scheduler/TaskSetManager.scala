 ...
 */
// 跟踪每个task，如果有失败的task就根据重试次数重新提交
// 还会处理task部署的位置感知,为executor添加taskSet等
private[spark] class TaskSetManager(
  ...
  {
    // 遍历出指定host上executor所有需要执行的task，如果该executor上没有需要执行的task 返回为None
  ...
  private def getAllowedLocalityLevel(curTime: Long): TaskLocality.TaskLocality = {
    ...
    def moreTasksToRunIn(pendingTasks: HashMap[String, ArrayBuffer[Int]]): Boolean = {
      ...
      // 根据本地级别的不同：开始遍历每个executor或者host或者Rack上的所有需要执行的task
      val hasTasks = pendingTasks.exists {
        case (id: String, tasks: ArrayBuffer[Int]) =>
          // 判断这个executor或者host或者Rack中所有的tasks是否可用
          if (tasksNeedToBeScheduledFrom(tasks)) {
            true
          } else {
            // 不可以则加入emptyKeys 返回false
            emptyKeys += id
            false
          }
      }
      // The key could be executorId, host or rackId
      // 若emptyKeys有值就 提取出来 并从pendingTasksForExecutor中移除
    ...
    while (currentLocalityIndex < myLocalityLevels.length - 1) {
      val moreTasks = myLocalityLevels(currentLocalityIndex) match {
          // pendingTasksForExecutor中维护的是每个executor上需要执行的Task
      ...
      if (!moreTasks) {
        ...
        // 如果有本地级别的Task 并且超过了对应级别的等待时间，就直接跳到下一个级别
  ...
  private def getLocalityWait(level: TaskLocality.TaskLocality): Long = {
    ...
    if (localityWaitKey != null) {
      // 如果之前设置过本地级级别的等待时间就用设置过的，否则用默认的
